...
import { listarMedicamentos, crearMedicamento } from '../../../services/medicamento.service'
...
export const GET = withAuth(async (request: NextRequest, user: AuthUser) => {
  try {
    ...
    if (user.rol === 'superadmin') {
      ...
      const countRes = await db.request().query<{ total: number }>(`SELECT COUNT(*) AS total FROM medicamento WHERE activo=1 ${w}`)
      const total = countRes.recordset[0]?.total ?? 0
      const res = await db.request()
        .input('offset', sql.default.Int, pag.offset).input('limit', sql.default.Int, pag.limit)
        .query(`SELECT id_medicamento,nombre,descripcion,activo,codigo_pais FROM medicamento WHERE activo=1 ${w} ORDER BY codigo_pais,nombre OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY`)
    ...
    const result = await listarMedicamentos(getNodeDb(user.codigo_pais), user.codigo_pais, pag)
...
export const POST = withAuth(async (request: NextRequest, user: AuthUser) => {
  try {
    ...
    if (!body.nombre) return NextResponse.json({ success: false, error: 'nombre es requerido' }, { status: 400 })
    ...
    const { getNodeDb } = await import('../../../lib/db')
    const data = await crearMedicamento(getNodeDb(codigo_pais), { nombre: body.nombre, descripcion: body.descripcion }, codigo_pais)
