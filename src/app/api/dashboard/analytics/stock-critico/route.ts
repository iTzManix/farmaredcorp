...
export const GET = withAuth(async (request: NextRequest, user: AuthUser) => {
  try {
    ...
    const umbral = parseInt(url.searchParams.get('umbral') ?? '10')
    ...
    return NextResponse.json({ success: true, data, meta: { umbral, codigo_pais: cp ?? 'ALL', total_alertas: data.length } })
  } catch (err) {
