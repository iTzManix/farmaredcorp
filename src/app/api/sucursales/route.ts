// src/app/api/sucursales/route.ts
import { NextRequest, NextResponse } from 'next/server'
import { withAuth } from '../../../middleware/auth.middleware'
import { listarSucursales, crearSucursal } from '../../../services/sucursal.service'
...
export const GET = withAuth(async (request: NextRequest, user: AuthUser) => {
  try {
    ...
    if (user.rol === 'superadmin') {
      ...
      const countRes = await db.request().query<{ total: number }>(`SELECT COUNT(*) AS total FROM sucursal WHERE activo=1 ${w}`)
      const total = countRes.recordset[0]?.total ?? 0
      const res = await db.request()
        .input('offset', sql.default.Int, pag.offset).input('limit', sql.default.Int, pag.limit)
        .query(`SELECT id_sucursal,nombre,direccion,activo,codigo_pais FROM sucursal WHERE activo=1 ${w} ORDER BY nombre OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY`)
    ...
    const result = await listarSucursales(getNodeDb(user.codigo_pais), user.codigo_pais, pag)
...
export const POST = withAuth(async (request: NextRequest, user: AuthUser) => {
  try {
    ...
    if (!body.nombre || !body.direccion)
      return NextResponse.json({ success: false, error: 'nombre y direccion son requeridos' }, { status: 400 })
    ...
    const { getNodeDb } = await import('../../../lib/db')
    const data = await crearSucursal(getNodeDb(codigo_pais), { nombre: body.nombre, direccion: body.direccion }, codigo_pais)
